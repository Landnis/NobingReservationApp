...
var hotelTitle = dataImages.map({$0.title})
var hotelImages = dataImages.map({$0.images})
...
func constraintLayout() {
    ...
    titleStackView.trailingAnchor.constraint(equalTo:  collectionView!.trailingAnchor, constant: -10).isActive = true
...
extension HotelSelectedViewController: UICollectionViewDataSource,UICollectionViewDelegate,UICollectionViewDelegateFlowLayout {
func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
   // let i = imageTitle.firstIndex(of: titleLabel.text)!
   // return imagesNames[i]!.count
    let i = hotelTitle.firstIndex(of: titleLabel.text)!
    return hotelImages[i]!.count
}
