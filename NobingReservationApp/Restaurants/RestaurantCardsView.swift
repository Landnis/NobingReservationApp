...
protocol RestaurantCardsDelegate:AnyObject {
    func restaurantCardTapped(title:String,image:UIImage,subTitle:String,phone:String,price:Float)
}

class RestaurantCardsView :MDCCardCollectionCell {
    weak var delegate: RestaurantCardsDelegate?
    var price: Float!
    ...
    let cardButton: MDCButton = {
        ...
        button.setTitle("Get A Reservation", for: .normal)
    ...
    @objc func cardButtonTouchUpInsideHandler(_ sender: MDCButton) {
        delegate?.restaurantCardTapped(title: labelCard.text!,image: imageCard.image!,subTitle: subLabel.text!,phone: phone.text!,price:price)
